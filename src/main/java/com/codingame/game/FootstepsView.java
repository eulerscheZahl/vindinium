...
public class FootstepsView implements IView {
	...
	public FootstepsView(List<Hero> heroes, GraphicEntityModule entityModule, Group boardGroup) {
		...
		footSteps = entityModule.createSpriteSheetSplitter().setSourceImage("footprints.png").setName("foot").setWidth(24).setHeight(24).setImageCount(1 * 2).setImagesPerRow(2).setOrigCol(0).setOrigRow(0).split();
	...
	public Sprite getStep(boolean bloody) {
		if (_footSteps.size() == 0) {
			Sprite steps = _entityModule.createSprite().setImage(footSteps[0]).setZIndex(-1).setAnchor(0.5);
	...
	public void onRound(int round) {
		...
		for (Hero hero : _heroes) {
			...
			step.setAlpha(1, Curve.NONE).setRotation(dir, Curve.NONE);
			newSteps.add(step);
		}

...
public class Config {
	...
	public static Board generateMap(List<Player> players, Properties params) {
		size = 10 + 2 * random.nextInt(8);
		wallPercent = 10 + random.nextInt(32);
		minePercent = 3 + random.nextInt(7);

		try {
			if (params.containsKey("size"))
				size = Integer.parseInt(params.getProperty("size"));
			if (params.containsKey("wallPercent"))
				wallPercent = Integer.parseInt(params.getProperty("wallPercent"));
			if (params.containsKey("minePercent"))
				minePercent = Integer.parseInt(params.getProperty("minePercent"));
			size += size % 2;
			if (size < 10)
				size = 10;
			if (size > 24)
		...
		while (true) {
			...
			if (board.countMines() == 0)
				continue;
			if (!placeTaverns(board, spawnPos))
				continue;
			while (board.shrink())
				size = board.size;
	...
	private static Board generateBoard() {
		...
		for (int x = 0; x < size / 2; x++) {
			for (int y = 0; y < size / 2; y++) {
				...
				if (rnd < minePercent)
					grid[x][y] = new Tile(Tile.Type.Mine, x, y);
				else if (rnd < minePercent + wallPercent)
					grid[x][y] = new Tile(Tile.Type.Wall, x, y);
				else
					grid[x][y] = new Tile(Tile.Type.Air, x, y);
		...
		for (int x = 0; x < size; x++) {
			for (int y = 0; y < size; y++) {
				...
				if (t.type == Tile.Type.Mine) {
					if (!neighborReachable(board, reachable, t))
	...
	private static int reachableMines(Board board, Tile spawn) {
		...
		for (int x = 0; x < size; x++) {
			for (int y = 0; y < size; y++) {
				Tile t = board.tiles[x][y];
				if (t.type == Tile.Type.Mine && neighborReachable(board, reachable, t))
					result++;
			}
		}
	...
	private static boolean placeTaverns(Board board, Tile spawn) {
		...
		for (int x = 0; x < size; x++) {
			for (int y = 0; y < size; y++) {
				if (reachable[x][y])
					reachableFields.add(board.tiles[x][y]);
			}
		}
		...
		while (reachableFields.size() > 0) {
			...
			for (int x = 0; x < size; x++) {
				for (int y = 0; y < size; y++) {
					if (reachable2[x][y])
						reachableFields2.add(board.tiles[x][y]);
				}
			}
			int reachableSize2 = reachableFields2.size();

			if (reachableSize2 == reachableSize - 4 && mines == mines2)
				return true;
